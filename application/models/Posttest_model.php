...
class Posttest_model extends CI_Model
{
    ...
    public function savePosttestResult($data)
    {
        ...
        $query = $this->db->query("SELECT id_ps FROM tb_prepost WHERE id_soal='$nomor' AND id_test = 2");
        //kembalikan id_ps
        return $query->row_array();
        
    }
    public function getCountIndikatorPS1(){
        $query = $this->db->query("SELECT COUNT(id_ps) as jumlah FROM tb_prepost WHERE id_ps = 1 AND id_test = 2");
        return $query->row_array();
        
    }
    public function getCountIndikatorPS2(){
        $query = $this->db->query("SELECT COUNT(id_ps) as jumlah FROM tb_prepost WHERE id_ps = 2 AND id_test = 2");
        return $query->row_array();
    }
    public function getCountIndikatorPS3(){
        $query = $this->db->query("SELECT COUNT(id_ps) as jumlah FROM tb_prepost WHERE id_ps = 3 AND id_test = 2");
        return $query->row_array();
    }
    public function getCountIndikatorPS4(){
        $query = $this->db->query("SELECT COUNT(id_ps) as jumlah FROM tb_prepost WHERE id_ps = 4 AND id_test = 2");
        return $query->row_array();
    }
}
